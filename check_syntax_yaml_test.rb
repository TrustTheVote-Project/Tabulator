...
# Tabulator.  Call schema_process to validate each significant schema and then
# optionally write the schema to a file for later use by the Tabulator, for
# syntax-checking CDF datasets.  The schemas processed are:
...
# subsumed by a higher-level schema:
...
# calls to schema_check_syntax_error, each of which tests for a different
...
# schema_check_syntax, each of which uses data stored in files under
...
# variations.
  ...
  def test_check_syntax
    ...
    schema_check_syntax_error(trace, "unknown_type", false, 0)
    schema_check_syntax_error(trace, "unknown_string", false, 1)
    schema_check_syntax_error(trace, "question_info", true, 2)
    schema_check_syntax_error(trace, "test_opt", true, 3)
    schema_check_syntax_error(trace, "test_alt", true, 4)
    schema_check_syntax_error(trace, "district_info", true, 5)
    schema_check_syntax_error(trace, "precount_info", true, 6)
    schema_check_syntax_error(trace, "audit_trail", true, 7)
    schema_check_syntax_error(trace, "question_info", true, 2, 8)
    schema_check_syntax_error(trace, "invalid_hash_key", false, 9)
    schema_check_syntax_error(trace, "contest_info", true, 10)
    schema_check_syntax_error(trace, "test_opt", true, 3, 11)
    schema_check_syntax_error(trace, "test_opt", true, 3, 11, 12)
    schema_check_syntax_error(trace, "test_alt", true, 13)
    schema_check_syntax_error(trace, "test_alt", true, 4, 11, 14)
    schema_check_syntax_error(trace, "test_alt", true, 15)
    schema_check_syntax(trace, "test_opt")
    schema_check_syntax(trace, "test_alt")
    schema_check_syntax(trace, "district_info")
    schema_check_syntax(trace, "precinct_info")
    schema_check_syntax(trace, "precount_info")
    schema_check_syntax(trace, "contest_info")
    schema_check_syntax(trace, "candidate_info")
    schema_check_syntax(trace, "question_info")
    schema_check_syntax(trace, "counter_info")
    schema_check_syntax(trace, "election_definition")
    schema_check_syntax(trace, "answer_count")
    schema_check_syntax(trace, "question_count")
    schema_check_syntax(trace, "candidate_count")
    schema_check_syntax(trace, "contest_count")
    schema_check_syntax(trace, "audit_trail")
    schema_check_syntax(trace, "audit_trail", "_hardware")
    schema_check_syntax(trace, "audit_trail", "_provenance")
    schema_check_syntax(trace, "audit_trail", "_nil_provenance")
    schema_check_syntax(trace, "audit_trail", "_all")
    schema_check_syntax(trace, "counter_count")
    schema_check_syntax(trace, "tabulator_count")
  end
  
# schema_setup
#
# * <i>trace</i>:  [type Integer] limits tracing of output for syntax checker
...
# Returns: N/A
#
# Assert (test) the validity of the <i>schema</i>, unless <i>check</i> is
# <i>false</i> (for testing syntax errors involving invalid schemas).  Schemas
# are stored in the Schemas/ subdirectory and are named "schema_foo.yml" when
# the schema is for an object named "foo".  If the schema file does not yet
# exist, it is written.  If it does exist, but the <i>write</i> flag is
# <i>true</i>, then it is over-written.  Deleting schema files from the
# Schemas/ directory permits these schemas to be re-generated when the unit
# tests are run.

  def schema_setup(trace, prefix, schema, check = true, write = false)
    print "Checking Validity of Schema: #{prefix}\n" if check
    assert(CheckSyntaxYaml.new.schema_is_valid?(schema, trace),
           "Invalid Schema: #{schema.inspect}") if check
...
# (under Tests/Syntax/), and then assert (test) the success of performing a
# syntax check of the datum against the schema.

  def schema_check_syntax(trace, prefix, extra = '')
    file = "Schemas/" + "#{prefix}_schema.yml"
    ...
    assert(CheckSyntaxYaml.new.check_syntax(schema, datum, trace),
...
# * <i>trace</i>:     [type Integer] limits tracing of output for syntax checker
# * <i>prefix</i>:    [type String] prefix of the schema and data file names
# * <i>validate</i>:  [type Boolean] indicates when the schema is to be validated
...
# error from another file (under Tests/Syntax/Errors/), and then assert (test)
# the failure of performing a syntax check of the datum against the schema.
# If <i>validate</i> is <i>false</i>, the schema validation check is bypassed
# during the syntax check.  We are testing for errors, so the syntax check
# must fail, and the resulting error code stack's first three errors must be
# <i>err1</i>, <i>err2</i> (optional), and <i>err3</i> (optional).

  def schema_check_syntax_error(trace, prefix, validate, err1, err2 = '', err3 = '')
    file = "Schemas/" + "#{prefix}_schema.yml"
    print "Reading Schema File: #{file}\n"
    schema = File.open(file) { |infile| YAML::load(infile) }
    ...
    if (validate)
      assert(!csy.check_syntax(schema, datum, trace),
             "Check Syntax of #{prefix} did not FAIL, but SHOULD")
    else
      assert(!csy.check_syntax_test(schema, datum, trace),
             "Check Syntax of #{prefix} did not FAIL, but SHOULD")
    end
    errstack3 = csy.error_stack[2].inspect
    errstack2 = csy.error_stack[1].inspect
    errstack1 = csy.error_stack[0].inspect
    assert((err3 == csy.error_stack[2]),
           "3rd error code (#{errstack3}) must be: #{err3}") unless (err3 == '')
    assert((err2 == csy.error_stack[1]),
           "2nd error code (#{errstack2}) must be: #{err2}") unless (err2 == '')
    assert((err1 == csy.error_stack[0]),
           "1st error code (#{errstack1}) must be: #{err1}")
  end
