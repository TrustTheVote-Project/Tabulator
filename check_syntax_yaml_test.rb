...
# test_check_syntax
#
# First test the errors generated by the syntax checker, by asserting the
# success of repeated calls to schema_check_syntax_error, each of which tests
# for a different error code, and where all possible error codes are covered.
#
# Next test the syntax of all significant schemas, by asserting the success of
...
# setup.  Some schemas, especially those with optional Hash keys, have
# multiple test variations.

  def test_check_syntax
    trace = 300
    assert schema_check_syntax_error(trace, "unknown_type", true, 0)
    assert schema_check_syntax_error(trace, "unknown_string", true, 1)
    assert schema_check_syntax_error(trace, "question_info", false, 2)
    assert schema_check_syntax_error(trace, "test_opt", false, 3)
    assert schema_check_syntax_error(trace, "test_alt", false, 4)
    assert schema_check_syntax_error(trace, "district_info", false, 5)
    assert schema_check_syntax_error(trace, "precount_info", false, 6)
    assert schema_check_syntax_error(trace, "audit_trail", false, 7)
    assert schema_check_syntax_error(trace, "question_info", false, 2, 8)
    assert schema_check_syntax_error(trace, "invalid_hash_key", true, 9)
    assert schema_check_syntax_error(trace, "contest_info", false, 10)
    assert schema_check_syntax_error(trace, "test_opt", false, 3, 11)
    assert schema_check_syntax_error(trace, "test_opt", false, 3, 11, 12)
    assert schema_check_syntax_error(trace, "test_alt", false, 13)
    assert schema_check_syntax_error(trace, "test_alt", false, 4, 11, 14)
    assert schema_check_syntax_error(trace, "test_alt", false, 15)
    assert schema_check_syntax(trace, "test_opt")
    assert schema_check_syntax(trace, "test_alt")
    assert schema_check_syntax(trace, "district_info")
    assert schema_check_syntax(trace, "precinct_info")
    assert schema_check_syntax(trace, "precount_info")
    assert schema_check_syntax(trace, "contest_info")
    assert schema_check_syntax(trace, "candidate_info")
    assert schema_check_syntax(trace, "question_info")
    assert schema_check_syntax(trace, "counter_info")
    assert schema_check_syntax(trace, "election_definition")
    assert schema_check_syntax(trace, "answer_count")
    assert schema_check_syntax(trace, "question_count")
    assert schema_check_syntax(trace, "candidate_count")
    assert schema_check_syntax(trace, "contest_count")
    assert schema_check_syntax(trace, "audit_trail")
    assert schema_check_syntax(trace, "audit_trail", "_hardware")
    assert schema_check_syntax(trace, "audit_trail", "_provenance")
    assert schema_check_syntax(trace, "audit_trail", "_nil_provenance")
    assert schema_check_syntax(trace, "audit_trail", "_all")
    assert schema_check_syntax(trace, "counter_count")
    assert schema_check_syntax(trace, "tabulator_count")
  end
...
# re-generated when these unit tests are run.
  ...
  def schema_setup(trace, prefix, schema, check = true, write = false)
    if (check)
      ...
      unless CheckSyntaxYaml.new.schema_is_valid?(schema, trace)
...
# against the schema. 
  ...
  def schema_check_syntax(trace, prefix, extra = '')
    ...
    if (! CheckSyntaxYaml.new.check_syntax(schema, datum, trace))
...
# check fails and generates the expected errors.
  ...
  def schema_check_syntax_error(trace, prefix, noval, err1, err2 = '', err3 = '')
    file = "Schemas/" + "#{prefix}_schema.yml"
    print "Reading Schema File: #{file}\n"
    schema = File.open(file) { |infile| YAML::load(infile) }
    ...
    if (noval ? cs.check_syntax_test(schema, datum, trace) : cs.check_syntax(schema, datum, trace))
